...
class KGWUtils:
    ...
    def _f_time(self, input_ids: torch.LongTensor):
        ...
        for i in range(0, self.config.prefix_length):
            ...
            # print(input_ids)
            # print("-1-i",-1-i)
            # print("prefix_length",self.config.prefix_length)
    ...
    def _get_greenlist_ids_left(self, input_ids: torch.LongTensor) -> list[int]:
        ...
        # for i in range(len(vocab_permutation)):
        #     if vocab_permutation[i]>self.config.vocab_size:
        #         print("\nwrong\n")
        #         break
        #     if i==len(vocab_permutation)-1:
        #         print("\ncorrect\n")
        #         break
        # print("\nget")
        # print(vocab_permutation[:greenlist_size])
        greenlist_ids = vocab_permutation[:greenlist_size]
        # for i in range(len(greenlist_ids)):
        #     print(greenlist_ids[i])
...
class KGWLogitsProcessor(LogitsProcessor):
    ...
    def _calc_greenlist_mask(self, scores: torch.FloatTensor, greenlist_token_ids: torch.LongTensor) -> torch.BoolTensor:
        ...
        # print("shape_scores",scores.shape)
        # print("shape_gti",len(greenlist_token_ids),len(greenlist_token_ids[0]))
        for b_idx in range(len(greenlist_token_ids)):
            # print("\npretry")
            green_tokens_mask[b_idx][greenlist_token_ids[b_idx]] = 1
            # for i in range(len(greenlist_token_ids[b_idx])):
            #     green_tokens_mask[b_idx][greenlist_token_ids[b_idx][i]] = 1
            # print("\ntry")
