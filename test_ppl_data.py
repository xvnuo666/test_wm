...
# Load data
with open('/home/xunuo2024/xn_code/ubmc/data/finance_qa_phi.jsonl', 'r') as f:
    lines = f.readlines()
...
for i in range(test_len):
    text=my_watermark.generate_watermarked_text(prompt=lines[i]['instruction'])
    # inputs=lines[i]['prompt']+lines[i]['natural_text']
    inputs = lines[i]['instruction']
    inputs = transformers_config.tokenizer(inputs, return_tensors='pt').to(device)
    #  inputs=transformers_config.tokenizer(inputs, return_tensors='pt').to(device)
    unwatermark_output = model.generate(**inputs, max_new_tokens=200, min_length=230, do_sample=False,no_repeat_ngram_size=4)
    generated_text = transformers_config.tokenizer.decode(unwatermark_output[0], skip_special_tokens=True)
    # generated_text=lines[i]['output']
    ...
    print("cos:",cos)
print("p1_natural:{},p2_watermarked:{},cos:{}".format(p1,p2,cos/test_len))