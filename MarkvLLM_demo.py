...
def main(algorithm_name, model_path):
    ...
    # 加载模型和分词器，并移动到 GPU 2
    model_for_causal_lm = AutoModelForCausalLM.from_pretrained(model_path).to(device)
    ...
    nowatermarked_img.save(f"demo_markvllm_{algorithm_name}_watermarked.png")
    watermarked_img = visualizer.visualize(
        data=watermark.get_data_for_visualization(text=watermark_text[0]),
        show_text=True, visualize_weight=True, display_legend=True
    )
    watermarked_img.save(f"demo_markvllm_{algorithm_name}_watermarked.png")
